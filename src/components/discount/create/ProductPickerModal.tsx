...
import type { ShopProduct } from '../../../services/market/products.repo'
...
type ProductPickerModalProps = {
  ...
  onConfirmSelection: (nextSelection: string[]) => void
}
...
function ProductPickerModal({
  ...
  const allFilteredSelected =
    filteredProductIds.length > 0 && selectedInFilteredCount === filteredProductIds.length
  ...
  useEffect(() => {
    ...
    selectAllRef.current.indeterminate = selectionMode === 'multiple' && someFilteredSelected
  }, [selectionMode, someFilteredSelected])
  ...
  const handleToggleSelectAll = () => {
    ...
    if (allFilteredSelected) {
      setDraftSelection((previous) => previous.filter((id) => !filteredProductIds.includes(id)))
      return
    }
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex min-w-0 items-start gap-2">
                      ...
                      <div className="min-w-0">
                        <h3 className="text-base font-semibold text-[#1E40AF] sm:text-xl">{title}</h3>
                        <p className="mt-0.5 text-[11px] text-slate-500 sm:hidden">{subtitle}</p>
                      </div>
                    </div>
                    <button
                      type="button"
